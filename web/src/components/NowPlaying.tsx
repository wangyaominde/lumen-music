...
export function NowPlaying() {
  ...
  // Shared with PlayerBar via the ['favoriteIds'] cache — no duplicate fetch
  // per track change.
  ...
  const favorited = !!track && !!favoriteIds?.includes(track.id);

  useEffect(() => {
    if (!track?.album_id) { setPalette(null); return; }
    let cancelled = false;
    // 320px variant: same request the blurred backdrop uses, so it's cached.
    extractPalette(coverUrl(track.album_id, true, 320))
  ...
  const toggleFavorite = async () => {
    if (!track) return;
    const id = track.id;
    if (favorited) await api.removeFavorite(id);
    else await api.addFavorite(id);
    qc.setQueryData<number[]>(['favoriteIds'], ids =>
      favorited ? (ids ?? []).filter(x => x !== id) : [...(ids ?? []), id]
    );
  ...
  return (
    <AnimatePresence>
      {open && track && (
        ...
        >
          {/* Animated gradient background (drift is desktop-only, see styles.css) */}
          <div className="absolute inset-0 -z-10">
            ...
            {track.album_id && (
              <img
                src={coverUrl(track.album_id, true, 320)}
                alt=""
                decoding="async"
          ...
          >
            <div className="flex flex-col items-center justify-center min-h-0">
              ...
              <div className="mt-6 sm:mt-8 w-full max-w-[640px] text-center">
                ...
                <SeekBar
                  className="mt-5 sm:mt-7 gap-3"
                  timeColor="rgba(255,255,255,0.7)"
                  trackColor="rgba(255,255,255,0.95)"
                  timeClass="w-10"
                />
...
/**
 * Lyric sync lives in its own leaf: the selector returns the active line
 * index, so timeupdates only re-render this panel when the highlighted line
 * actually changes — never the whole overlay.
 */
function LyricsPanel({ lrc }: { lrc: LrcLine[] | null }) {
  const lyricsRef = useRef<HTMLDivElement | null>(null);
  const lrcIdx = usePlayer(s => (lrc && lrc.length > 0 ? activeLrcIndex(lrc, s.currentTime + 0.15) : -1));
